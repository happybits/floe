...
if [ $? -ne 0 ]; then
    if [ "$os" == "Darwin" ]; then
        echo >&2 "mysqld missing - installing ..."
        brew update && brew install mysql || exit 1
    else
        echo >&2 "mysqld missing - please install"
        exit 1
    fi
fi
...
if [ ! -f "$venv_dir/bin/python" ]; then
    ...
    virtualenv -p python3 -q "$venv_dir" || {
        echo >&2 "unable to configure the virtualenv for the project in $venv_dir"
        exit 1
    }
fi
...
if [ $? -ne 0 ]; then
    echo >&2 "failed to install pip packages"
    exit 1
fi
